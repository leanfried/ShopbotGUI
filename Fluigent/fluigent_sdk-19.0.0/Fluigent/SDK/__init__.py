...
def fgt_get_controllersInfo():
    ...
    exceptions.manage_generic_status(low_level_function.__name__, c_error)
    return [fgt_CONTROLLER_INFO(c) for c in channels]
...
def fgt_get_pressureChannelsInfo():
    ...
    c_error, channels = low_level_function()
    exceptions.manage_generic_status(low_level_function.__name__, c_error)
    return [fgt_CHANNEL_INFO(c) for c in channels]
...
def fgt_set_pressureUnit(pressure_index, unit):
    ...
    low_level_function(pressure_index, unit)
...
def fgt_set_sensorUnit(sensor_index, unit):
    ...
    low_level_function(sensor_index, unit)
...
def fgt_get_sensorCalibration(sensor_index):
    ...
    exceptions.manage_sensor_status(low_level_function.__name__, sensor_index)
    return fgt_SENSOR_CALIBRATION(calibration)
...
def fgt_set_sensorCustomScale(sensor_index, a, b = 0, c = 0, smax = None):
    ...
    sensor_index = int(sensor_index)
    if smax is None:
        low_level.fgt_set_sensorCustomScale(sensor_index, a, b, c)
    else:
        low_level.fgt_set_sensorCustomScaleEx(sensor_index, a, b, c, smax)
...
def fgt_calibratePressure(pressure_index):
    ...
    low_level_function(pressure_index)
    exceptions.manage_pressure_status(low_level_function.__name__, pressure_index)
...
def fgt_set_pressure(pressure_index, pressure):
    ...
    low_level_function(pressure_index, pressure)
    exceptions.manage_pressure_status(low_level_function.__name__, pressure_index)
    
def fgt_get_pressure(pressure_index, include_timestamp = False):
...
def fgt_set_sensorRegulation(sensor_index, pressure_index, setpoint):
    ...
    low_level_function(sensor_index, pressure_index, setpoint)
    exceptions.manage_sensor_status(low_level_function.__name__, sensor_index)
    
def fgt_get_sensorValue(sensor_index, include_timestamp = False):
    ...
    if include_timestamp:
        ...
        return sensorMeasurement, timestamp
    else:
...
def fgt_set_pressureLimit(pressure_index, p_lim_min, p_lim_max):
    ...
    low_level_function(pressure_index, p_lim_min, p_lim_max)
    exceptions.manage_pressure_status(low_level_function.__name__, pressure_index)
...
def fgt_set_sensorRegulationResponse(sensor_index, response_time):
    ...
    low_level_function(sensor_index, response_time)
    exceptions.manage_sensor_status(low_level_function.__name__, sensor_index)
...
def fgt_set_pressureResponse(pressure_index, response_mode):
    ...
    low_level_function(pressure_index, response_mode)
    exceptions.manage_pressure_status(low_level_function.__name__, pressure_index)
...
def fgt_set_manual(pressure_index, voltage):
    ...
    low_level_function(pressure_index, voltage)
    exceptions.manage_pressure_status(low_level_function.__name__, pressure_index)
